...
#define MAX_WAIT_TIME     30000		// 30 milli sec
	...
	//printf("PTS=%s \n", szMsg);
	if(!IsNewFrame(pCtx->codingType, pBuffer->pBuffer, pBuffer->nFilledLen)) {
...
#ifdef WIN32
		WaitForSingleObject(pCtx->readThrdId, 0);
#else
...
void fakedecSetInputConn(StrmCompIf *pComp, int nConnNum, ConnCtxT *pConn)
{
	FakeDecCtx *pAppData = (FakeDecCtx *)pComp->pCtx;
	pAppData->pConnSrc = pConn;
}

void fakedecSetClkSrc(StrmCompIf *pComp, void *pClk)
{
	FakeDecCtx *pAppData = (FakeDecCtx *)pComp->pCtx;
	pAppData->pClk = pClk;
}
