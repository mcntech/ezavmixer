	...
	if(strncmp(pszUdpSrc,UDP_PROTOCOL,strlen(UDP_PROTOCOL)) == 0){
		...
		if(pIndx) {
			...
			if(pIndx == pTmp) {
				...
				// Do nothing
				JdDbg(CJdDbg::DBGLVL_SETUP, ("Address not specified."));
			} else {
...
#endif
	JdDbg(CJdDbg::DBGLVL_SETUP, ("addrFirstByte=%d addr=0x%x", addrFirstByte, addr->s_addr));
...
int OpenIpcSocket(char *szSockName)
{
	...
	int sock;
	// Open IPC Socket
	JdDbg(CJdDbg::DBGLVL_SETUP, ("Opening IPC Socket %s", szSockName));
	...
	if (bind(sock, (sockaddr *)&ipc_sock_addr, len) < 0) {
...
int udprxOpen(StrmCompIf *pSrc, const char *szUdpSrc)
{
	...
	if(strcmp(szUdpSrc, TS_IPC_SOCKET) == 0) {
		...
		if(res < 0) {
			JdDbg(CJdDbg::DBGLVL_ERROR, ("Failed to parse %s", szUdpSrc));
			return -1;
		}
	#ifdef WIN32
		{
		...
		if(sock == -1) {  
			JdDbg(CJdDbg::DBGLVL_ERROR, ("Failed to create socket"));
			return -1;
		}
		...
		if ( setsockopt(sock, SOL_SOCKET, SO_RCVBUF, (char*)&windowSize, windowSizeLen) == -1 )  {
			JdDbg(CJdDbg::DBGLVL_ERROR, ("setsockopt Failed"));
			return -1;
		}
		...
		if(pCtx->fMulticast) {
			...
			JdDbg(CJdDbg::DBGLVL_SETUP, ("Joining Multicast group"));
			if ( setsockopt( sock, IPPROTO_IP, IP_ADD_MEMBERSHIP, (const char *)&mreq, sizeof(mreq)) < 0){
				fprintf(stderr, "IP_ADD_MEMBERSHIP Failed \n");
				JdDbg(CJdDbg::DBGLVL_ERROR, ("IP_ADD_MEMBERSHIP Failed"));
				return -1;
			}
		} else {
			JdDbg(CJdDbg::DBGLVL_SETUP, ("Listening on unicast port %d", pCtx->usRxPort));
		}
	}
	pCtx->mhSock = sock;
	printf("sock =%d\n", sock);
	...
	while(1) {
		...
		if(pCtx->nUiCmd == STRM_CMD_STOP) {
			...
			if(selectRet == 0) {
				...
				JdDbg(CJdDbg::DBGLVL_ERROR,("Socket select error on %d. Exiting Streaming Task.", pCtx->usRxPort));
				break;
			}
			...
			ShowStat(pCtx);
			//JdDbg(CJdDbg::DBGLVL_TRACE, ("Bytes read %d\n", ret));
		}
...
#endif
		JdDbg(CJdDbg::DBGLVL_SETUP, ("[pthread_join the thread: Begin"));
#ifdef WIN32
		WaitForSingleObject(pCtx->hThread,0);	return 0;
#else
		pthread_join (pCtx->hThread, (void **) &res);
#endif
		JdDbg(CJdDbg::DBGLVL_SETUP, ("pthread_join the thread: Done]"));
	}
...
int udprxSetInputConn(StrmCompIf *pComp, int nCOnnNum, ConnCtxT *pConn)
{
	return 0;
}
...
{
	UdpRxCtxT *pCtx = (UdpRxCtxT *)pComp->pCtx;
	pCtx->pConn = pConn;
	return 0;
}
