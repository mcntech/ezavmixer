...
namespace H264
{
  ...
  inline int32_t cBitReader::se(void)
  {
    ...
    uint32_t r = ue();
    if (r > 0xFFFFFFFE)
       Error("ERROR: H264::cBitReader::se(): overflow");
    //int n = (1 - 2 * (r & 1)) * ((r + 1) / 2);
	int n = (r % 2) ? (r / 2) : -(r / 2);
	return n;
  }
  ...
  inline void cContext::ActivateSPS(uint32_t ID)
  {
    if (ID >= (sizeof (spsStore) / sizeof (*spsStore)))
       Error("ERROR: H264::cContext::ActivateSPS(): id out of range");
    if (!spsStore[ID].Defined())
       Error("ERROR: H264::cContext::ActivateSPS(): requested SPS is undefined");
    sps = &spsStore[ID];
  }
  ...
  inline void cContext::ActivatePPS(uint32_t ID)
  {
    if (ID >= (sizeof (ppsStore) / sizeof (*ppsStore)))
       Error("ERROR: H264::cContext::ActivatePPS(): id out of range");
    if (!ppsStore[ID].Defined())
       Error("ERROR: H264::cContext::ActivatePPS(): requested PPS is undefined");
    pps = &ppsStore[ID];
  ...
  inline void cContext::Define(cSequenceParameterSet &SPS)
  {
    if (SPS.seq_parameter_set_id >= (sizeof (spsStore) / sizeof (*spsStore)))
       Error("ERROR: H264::cContext::DefineSPS(): id out of range");
    SPS.defined = true;
  ...
  inline void cContext::Define(cPictureParameterSet &PPS)
  {
    if (PPS.pic_parameter_set_id >= (sizeof (ppsStore) / sizeof (*ppsStore)))
       Error("ERROR: H264::cContext::DefinePPS(): id out of range");
    PPS.defined = true;
  ...
  public:
	...
	int ParseSequenceParameterSetMin(uint8_t *Data, int Count, long *plWidth, long *plHeight);
  };
}
