...
int CRtpSrvBridge::SendVideo(unsigned char *pData, int size, unsigned long lPts)
{
	long hr = 0;
	int nSearchLen;
	...
	//DumpHex(pData, 32);
	//DBG_PRINT("CRtpSrvBridge::SendVideo size=%d pts=%d(ms)\n", size, lPts / 1000);
	if(m_fDiscont) {
	...
	m_TsMux.Mux(&Au);
	m_Mutex.Acquire();
	if (m_pRtpSrv->Write(m_pTsBuffer, Au.m_TsSize) < 0 ) {
		hr = -1;
	}
	m_Mutex.Release();
	return hr;
...
int CRtpSrvBridge::SetVideoDiscont() 
{ 
	m_fDiscont = 1;
	return 0;
}

int CRtpSrvBridge::SendAudio(unsigned char *pData, int size, unsigned long lPts)
{
	long hr = 0;
	unsigned long ulFlags = 0;
	CAccessUnit Au;
...
int CRtpSrvBridge::Run(COutputStream *pOutputStream)
{
