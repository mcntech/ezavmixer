...
int WriteData(MpegTsDemuxCtx *pCtx, unsigned char *pData, int item_size, int length, int nStrmId)
{
	...
	DBG_LOG(DBGLVL_PACKET, ("strmid=%d length=%d",nStrmId, dataLen));
	if(nStrmId == STRM_ID_VID) {
		...
		DBG_LOG(DBGLVL_PACKET, ("Sending Vid ulFlags=0x%x", ulFlags));
		pCtx->pConnVidOut->Write(pCtx->pConnVidOut, pData, dataLen,  ulFlags, pCtx->crnt_vid_pts * 1000 / 90);
		pCtx->VidChunkCnt++;
	} else {
		if(pCtx->pConnAudOut) {
			unsigned long ulFlags = 0;
			...
			DBG_LOG(DBGLVL_PACKET, ("Sending Aud ulFlags=0x%x", ulFlags));
			pCtx->pConnAudOut->Write(pCtx->pConnAudOut, pData, dataLen,  ulFlags, pCtx->crnt_aud_pts * 1000 / 90);
		}
		pCtx->AudChunkCnt++;
	}
#endif
...
int demuxSetOption(StrmCompIf *pComp, int nCmd, char *pOptionData)
{
	...
	{ 
		case DEMUX_CMD_SELECT_PROGRAM:
		{
			...
			DBG_LOG(DBGLVL_SETUP, ("detectpids=%d prog=%d vhan=%d achan=%d", pCtx->detect_program_pids, pCtx->program, pCtx->video_channel, pCtx->audio_channel));
			DBG_LOG(DBGLVL_SETUP, ("pcr_pid=%d aud_pid=%d vid_pid=%d aud_strm_type=0x%x vid_strm_type=0x%x", pCtx->pcr_pid, pCtx->audio_pid, pCtx->video_pid,pCtx->audio_stream_type, pCtx->video_stream_type));
		}
		break;
	}
...
int demuxClose(StrmCompIf *pComp)
{
...
{
	MpegTsDemuxCtx *pCtx = pComp->pCtx;
	pCtx->pConnSrc = pConn;
	return 0;
}
...
static void threadDemux(void *threadsArg)
{
	...
	while (pCtx->fRun)	{
		...
		while(pCtx->pConnSrc->IsEmpty(pCtx->pConnSrc) && pCtx->nUiCmd != STRM_CMD_STOP){
			//DBG_LOG(DBGLVL_WARN,("start=%d end=%d",pCtx->pConnSrc->pdpCtx->start, pCtx->pConnSrc->pdpCtx->end))
#ifdef WIN32
			Sleep(1);
#else
			usleep(1000);
		...
		if(ulFlags & OMX_BUFFERFLAG_EOS) {
			...
			demux_mpeg2_transport(pCtx, length, pData);
		}
		...
		if(pCtx->fEoS) {
			pCtx->fRun = 0;
			DBG_LOG(DBGLVL_TRACE, ("Exiting Demux thread"));
		}
	}
	free(pData);
	DBG_LOG(DBGLVL_TRACE, ("Leave"));
...
int demuxStop(StrmCompIf *pComp)
{
	...
	if(pCtx->fRun) {
		...
		DBG_LOG(DBGLVL_TRACE, ("fStreaming=%d", pCtx->fRun));
		...
		DBG_LOG(DBGLVL_TRACE, ("fStreaming=%d nTimeOut rem=%d", pCtx->fRun, nTimeOut));
	}
	...
	for(i = 0; i < length; i++)  {
		...
		if (parse == 0x00000100)  {
			...
			if(look_for_gop == TRUE)  {
				...
				if(gop_found == FALSE)  {
					if(pCtx->parse_only == FALSE)  {
						...
						WriteData(pCtx, &gop_header, 1, 9, STRM_ID_VID);
			...
			if (first_sequence == FALSE)  {
				...
				if(pCtx->parse_only == FALSE)  {
					WriteData(pCtx, &header, 1, 3, STRM_ID_VID);
...
void	demux_mpeg2_transport_init(MpegTsDemuxCtx *pCtx)
{
	...
	pCtx->psip_ptr[0x1ffb] = malloc(sizeof(struct psip));
								...
								if (pCtx->psip_ptr[pCtx->pid]->psip_table_id == 0xc7 && pCtx->mgt_version_number != pCtx->mgt_last_version_number)  {
									...
									for (k = 0; k < pCtx->mgt_tables_defined; k++)  {
										...
										if (pCtx->mgt_table_type == 0x4)  {
											...
											pCtx->psip_ptr[pCtx->ett_pid] = malloc(sizeof(struct psip));
											...
											pCtx->psip_ptr[pCtx->eit0_pid] = malloc(sizeof(struct psip));
											...
											pCtx->psip_ptr[pCtx->eit1_pid] = malloc(sizeof(struct psip));
											...
											pCtx->psip_ptr[pCtx->eit2_pid] = malloc(sizeof(struct psip));
											...
											pCtx->psip_ptr[pCtx->eit3_pid] = malloc(sizeof(struct psip));
											...
											pCtx->psip_ptr[pCtx->ett0_pid] = malloc(sizeof(struct psip));
											...
											pCtx->psip_ptr[pCtx->ett1_pid] = malloc(sizeof(struct psip));
											...
											pCtx->psip_ptr[pCtx->ett2_pid] = malloc(sizeof(struct psip));
											...
											pCtx->psip_ptr[pCtx->ett3_pid] = malloc(sizeof(struct psip));
...
int demuxDelete(StrmCompIf *pComp)
{
	if(pComp->pCtx) {
		free(pComp->pCtx);
	}
