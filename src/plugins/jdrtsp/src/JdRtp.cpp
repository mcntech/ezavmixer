...
void CRtp::ParseHeader(char *pData, RTP_PKT_T *pHdr)
{
	...
	pHdr->usSeqNum = ((((unsigned short)pData[2] << 8) & 0xFF00) |
						(((unsigned short)pData[3]) & 0x00FF)
						);

	pHdr->ulTimeStamp = ((((unsigned long)pData[4] << 24)  & 0xFF000000)| 
		                 (((unsigned long)pData[5] << 16) & 0x00FF0000) | 
						 (((unsigned long)pData[6] << 8)  & 0x0000FF00) | 
						 (((unsigned long)pData[7]) & 0x000000FF)
						);
