...
int omxalPlayStream(ENGINE_T *pEngine, CAvmixInputStrm *pSession, XADataSource *pDataSrc1, XADataSource *pDataSrc2)
{
	...
	engineObject = pEngine->engineObject;
	engineEngine = pEngine->engineEngine;
	...
	result = (*playerExt)->SetConfiguration(playerExt, SESSION_ID, sizeof(int), &pSession->nCodecSessionId);
	assert(XA_RESULT_SUCCESS == result);
...
MIXER_SESSION_T *CreateOutputMixer(ENGINE_T *pEngine)
{
	...
	if(pEngine) {
		...
		pSession = (MIXER_SESSION_T *)malloc(sizeof(MIXER_SESSION_T));
...
ENGINE_T *omxalInit(const char *pszConfFile, DISP_WINDOW_LIST *pWndList, AUD_CHAN_LIST *pAChanList)
{
	...
	if(strcmp(szLibraryName,"none") == 0){
		DBG_PRINT("!!! Codec library not specified. Diabling AVMixer !!!\n");
		goto Exit;
	}
	...
	if(g_xaCreateEngine == NULL){
        DBG_PRINT("Error Locating xaCreateEngine in %s: %s\n", szLibraryName, dlerror());
        goto Exit;
    }

#endif

	pEngine = (ENGINE_T *)malloc(sizeof(ENGINE_T));
