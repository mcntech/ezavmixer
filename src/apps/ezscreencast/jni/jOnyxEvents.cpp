...
bool COnyxEvents::onNativeMessage(char *szTitle, char *szMsg)
{
	JNIEnv* env;
...
bool COnyxEvents::onRemoteNodeError(char *url, char *szErr)
{
	JNIEnv* env;
	safeAttach(&env);//must always call safeDetach() before returning
	jclass onyxApi = env->GetObjectClass(g_jniGlobalSelf);
...
bool COnyxEvents::safeAttach(JNIEnv** env){
	if(*env == NULL){
		JNIEnv envalloc;
		*env = &envalloc;
	}
