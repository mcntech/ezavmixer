...
CTsMux::CTsMux() 
{
	m_pMuxBuffer = (unsigned char  *)malloc(MUX_BUFF_SIZE);
	memset(m_pMuxBuffer, 0x00, MUX_BUFF_SIZE);
	m_pLMEM_SYS_MUX_TS_BASE                  =   m_pMuxBuffer;
	m_pPES_HEADER_FLAGS_VIDEO                = m_pLMEM_SYS_MUX_TS_BASE + 0x00;
	m_pPES_HEADER_FLAGS_AUDIO                = (m_pLMEM_SYS_MUX_TS_BASE + 0x80);
	m_pTS_HEADER_ADDRESS_UNIT_START_VIDEO    = (m_pLMEM_SYS_MUX_TS_BASE + 0x100);
	m_pTS_HEADER_ADDRESS_UNIT_CONTINUE_VIDEO = (m_pLMEM_SYS_MUX_TS_BASE + 0x104);
	m_pTS_HEADER_ADDRESS_UNIT_START_AUDIO    = (m_pLMEM_SYS_MUX_TS_BASE + 0x108);
	m_pTS_HEADER_ADDRESS_UNIT_CONTINUE_AUDIO = (m_pLMEM_SYS_MUX_TS_BASE + 0x10c);
	m_pTS_HEADER_ADDRESS_UNIT_START_PCM      = (m_pLMEM_SYS_MUX_TS_BASE + 0x110);
	m_pTS_HEADER_ADDRESS_UNIT_CONTINUE_PCM   = (m_pLMEM_SYS_MUX_TS_BASE + 0x114);
	m_pTS_HEADER_ADDRESS_UNIT_PTS            = (m_pLMEM_SYS_MUX_TS_BASE + 0x118);

	m_pTS_HEADER_ADDRESS_UNIT_START_PAT      = (m_pLMEM_SYS_MUX_TS_BASE + 0x11c);
	m_pTS_HEADER_ADDRESS_UNIT_START_PMT      = (m_pLMEM_SYS_MUX_TS_BASE + 0x120);
	m_pPAT_TABLE_ADDRESS                     = (m_pLMEM_SYS_MUX_TS_BASE + 0x124);
	m_pPMT_TABLE_ADDRESS                     = (m_pLMEM_SYS_MUX_TS_BASE + 0x180);

	m_pTS_NULL_PACKETS                       = (m_pLMEM_SYS_MUX_TS_BASE + 0x210);
	Init();
}
...
int CTsMux::Init() 
{
    GeneratePESHeaderFlags(m_pPES_HEADER_FLAGS_VIDEO, VIDEO_STREAM_ID);
    GeneratePESHeaderFlags(m_pPES_HEADER_FLAGS_AUDIO, AUDIO_STREAM_ID);
    GenerateTSHeader(m_pTS_HEADER_ADDRESS_UNIT_START_PAT, 1, PROGRAM_ASSOCIATION_TABLE_PID, 0); 
    GenerateTSHeader(m_pTS_HEADER_ADDRESS_UNIT_START_PMT, 1, PROGRAM_MAP_TABLE_PID, 0); 
    GenerateTSHeader(m_pTS_HEADER_ADDRESS_UNIT_START_VIDEO, 1, VIDEO_ELEMENTARY_PID, 1);
    GenerateTSHeader(m_pTS_HEADER_ADDRESS_UNIT_CONTINUE_VIDEO, 0, VIDEO_ELEMENTARY_PID,1);
    GenerateTSHeader(m_pTS_HEADER_ADDRESS_UNIT_START_AUDIO, 1, AUDIO_ELEMENTARY_PID, 1);
    GenerateTSHeader(m_pTS_HEADER_ADDRESS_UNIT_CONTINUE_AUDIO, 0, AUDIO_ELEMENTARY_PID,1);
    GenerateTSHeader(m_pTS_HEADER_ADDRESS_UNIT_START_PCM, 1, PCM_ELEMENTARY_PID, 1);
    GenerateTSHeader(m_pTS_HEADER_ADDRESS_UNIT_CONTINUE_PCM, 0, PCM_ELEMENTARY_PID,1);
    GenerateTSHeader(m_pTS_HEADER_ADDRESS_UNIT_PTS, 0, SYSTEM_PCR_PID, 0);
    GeneratePAT(m_pPAT_TABLE_ADDRESS);
    GeneratePMT(m_pPMT_TABLE_ADDRESS);
...
int CTsMux::pcr_packet(unsigned long long pts, char *bitBuffAddr, unsigned char counter) 
{
    int  packet_size = 0;
    int  padding = 0;
    char* localTSHeaderAddr = (char *)m_pTS_HEADER_ADDRESS_UNIT_PTS;
...
int CTsMux::AddAPesHeader(CAccessUnit *pAu, char *pBuffer, int nPlLen)
{
	...
	char* lmemPESAddr = (char *)m_pPES_HEADER_FLAGS_AUDIO;
...
int CTsMux::AddVPesHeader(CAccessUnit *pAu, char *pBuffer, int nPlLen)
{
    ...
    unsigned long long   dts = pAu->m_DTS;
	int nAddBytes = 0;
	char* lmemPESAddr = (char *)m_pPES_HEADER_FLAGS_VIDEO;
...
int CTsMux::Mux(CAccessUnit *pAu)
{
	...
	switch(streamType) {
		case SAMPLE_TYPE_H264:
			...
			lmemTSAddr =  (char *)m_pTS_HEADER_ADDRESS_UNIT_START_VIDEO;
			break;
		case SAMPLE_TYPE_AAC:
			DbgLog(LVL_TRACE, ("SAMPLE_TYPE_AAC"));
			lmemTSAddr =  (char *)m_pTS_HEADER_ADDRESS_UNIT_START_AUDIO;
			break;
	}
	...
	if((IsVideoStream || (IsAudioStream && IsAudioOnly) ) && 
		(mPSISent[streamID] == 0 || mPSIInterval[streamID] > PSI_PERIOD || pAu->m_Flags & FORCE_SEND_PSI)) {

		char* lmemTSPATAddr = (char *)m_pTS_HEADER_ADDRESS_UNIT_START_PAT;
		char* lmemTSPMTAddr = (char *)m_pTS_HEADER_ADDRESS_UNIT_START_PMT;

		char* lmemPATAddr = (char *)m_pPAT_TABLE_ADDRESS;
		char* lmemPMTAddr = (char *)m_pPMT_TABLE_ADDRESS;
		memset(bitBufPtr, 0x00, TS_PACKET_SIZE);
    ...
    while(AUBytes > 0) {
		...
		if((IsVideoStream && newPES_Video) || (IsAudioStream && newPES_Audio)){
			if( IsVideoStream && newPES_Video){
				...
				newPES_Video = false;
				lmemTSAddr =  (char *)m_pTS_HEADER_ADDRESS_UNIT_CONTINUE_VIDEO;
			} else if (IsAudioStream && newPES_Audio){
				memset(bitBufPtr, 0x00, TS_PACKET_SIZE);
				bytes = AddAPesHeader(pAu, bitBufPtr, AUBytes);
				newPES_Audio = false;
				lmemTSAddr =  (char *)m_pTS_HEADER_ADDRESS_UNIT_CONTINUE_AUDIO;
			}
			bitBufPtr += bytes; 
			pAu->m_TsSize += bytes;
