...
namespace H264
{
  ...
  void cSimpleBuffer::Del(int Count)
  {
    ...
    if (Count < avail)
	   ...
	   memcpy(data, data + Count, avail - Count);
    avail -= Count;
    gotten = 0;
  }
  ...
  int cParser::ParseSequenceParameterSetMin(uint8_t *Data, int Count, long *plWidth, long *plHeight)
  {
    cSequenceParameterSet SPS;
	
	// Following are used for video dimesnions
	uint32_t chroma_format_idc = 1; //default as per 7.4.2.1 Sequence parameter set RBSP semantics
	SPS.frame_mbs_only_flag = 0;
	uint32_t SubWidthC = 0;
	uint32_t SubHeightC = 0;
	uint32_t CropUnitX = 1;
	uint32_t CropUnitY = 2;
	...
	*plHeight = (pic_height_in_map_units_minus1 + 1) * 16;
    ...
    //if (!omitPicTiming) 
	{
       ...
       if (frame_cropping_flag) {
          ...
          uint32_t frame_crop_top_offset =  br.ue();
          uint32_t frame_crop_bottom_offset =  br.ue();
		  *plWidth -= (frame_crop_left_offset + frame_crop_right_offset) * CropUnitX;
		  *plHeight -= (frame_crop_top_offset + frame_crop_bottom_offset) * CropUnitY;
       }
       uint32_t vui_parameters_present_flag = br.u(1);
       if (vui_parameters_present_flag) {
          uint32_t aspect_ratio_info_present_flag = br.u(1);
