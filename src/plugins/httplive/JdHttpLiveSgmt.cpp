...
#endif
#define  MEM_FILE_DEFAULT_FRONT_CACHE   1
...
{
public:
	CSegmentInf(int nSuffix, int nDuration, int fDiscont)
	{
	  m_nSuffix = nSuffix; 
		...
		)
{
	...
	if(fDiscont) {
		strcat(szbuff, TAG_DISCONTINUITY);
	}
	sprintf(szTemp,	"#EXTINF:%d, no desc\r\n", nDuration);
	strcat(szbuff, szTemp);
	sprintf(szTemp,	"%s\r\n", szSegmentName, nSegment);
	strcat(szbuff, szTemp);
	return strlen(szbuff);
}
		...
		)
{
	...
	for (i=0; i < nReqSegments && nBytesTotal < nMaxLen ; i++){
		fDiscont = 0;
		nBytes = m3u8AddSegment(szbuff, szSegmentName, nDuration, nSeqStart + i, fDiscont);
		nBytesTotal += nBytes;
		if(szbuff)
			szbuff += nBytes;
	}
...
int GetM3u8PlayListFromSegmentList(
		std::list <CSegmentInf *>  *pSegmentList,
		char *szbuff,				//< Buffer to return the m3u8 play list
		char *szSegmentNamePrefix,
		int  nMaxLen,				//< Buffer size
		int  nSkipSegments,
		int  nReqSegments,			//< Number of requested segments in the playlist
		int  fHead,					//< Add Playlist header
		int  fTail					//< Add Playlist End
		)
{
	...
	if(pSegmentList->size() <  nSkipSegments)
		goto Exit;
	...
	for (std::list<CSegmentInf *>::iterator it = pSegmentList->begin(); it != pSegmentList->end(); ++it) {
		if(i < nSkipSegments){
			...
			if(fHead && nSegmentaAdded == 0) {
				nBytes = m3u8AddHeader(szbuff, pSegmentInf->m_nDuration / 1000, pSegmentInf->m_nSuffix);
				nBytesTotal += nBytes;
				if(szbuff)
					szbuff += nBytes;
			}

			nBytes = m3u8AddSegment(szbuff, szSegmentNamePrefix, pSegmentInf->m_nDuration / 1000, pSegmentInf->m_nSuffix, pSegmentInf->m_fDiscont);
			nBytesTotal += nBytes;
			if(szbuff)
				szbuff += nBytes;
			nSegmentaAdded++;
...
public:
	...
	int WriteFrameData(char *pData, int nLen, int fVideo, unsigned long ulFlags, unsigned long ulPtsMs)
	{
		...
		char *pPkt = pData;
		int fHasSps = ulFlags & HLS_FLAG_HAS_SPS;
		int fDisCont = ulFlags & HLS_FLAG_DISCONT;

		if(ulFlags & HLS_FLAG_DISCONT ){
...
public:
			...
			m_fLiveOnly(fLiveOnly)
	{
		...
		m_fSupportDiscont = 1;
		m_nBcastSegmentCount = NUM_SEGS_IN_BCAST_PLAYLIST;
		m_fDiscont = 0;
	}
...
void CHlsPublishMemFile::UpLoadBroadcastPlayList()
{
	...
	int nBcastSegmentCount = m_fLiveOnly ? m_nBcastSegmentCount : m_SegmentList.size();
	int nBcastFrontCache = m_fLiveOnly ? m_nBcastFrontCache : 0;
	int fTail = m_fLiveOnly ? 0 : 1;

	nLen = GetM3u8PlayListFromSegmentList(&m_SegmentList, buffer, m_pszM3u8BaseName, PLAYLIST_BUFFER_SIZE, nBcastFrontCache, nBcastSegmentCount, 1, fTail);
	if(nLen > 0) {
...
void CHlsPublishMemFile::UpdateSlidingWindow()
{
	JDBG_LOG(CJdDbg::LVL_TRACE,("Enter"));
	int nDelSegment = 0;

	// Add segment to the tail
	CSegmentInf *pSegmentInf = new CSegmentInf(m_nSegmentIndex, m_CrntSegmentDuration, m_fDiscont);
	m_SegmentList.push_back(pSegmentInf);
	m_fDiscont = 0;

	// Remove head segment
	if(m_fLiveOnly && m_SegmentList.size() > m_nBcastSegmentCount +  m_nBcastFrontCache + m_nBcastBackCache) {
	...
	if(m_SegmentList.size()  >=  m_nBcastSegmentCount + m_nBcastBackCache + m_nBcastFrontCache) {
		UpLoadBroadcastPlayList();
	}

	// delete unused segment
	if(m_fLiveOnly && nDelSegment) {
		char szTsFile[256];
 ...
 * httpliveWriteData is expored to encoder to supply the data
 */
int hlsWriteFrameData(void *pCtx, char *pData, int nLen, int fVideo, unsigned long ulFlags,  unsigned long ulPtsMs)
{
	JDBG_LOG(CJdDbg::LVL_TRACE,("Enter"));
	int res = 0;
	CHlsSegmnter *pHttpLiveSgmnt = (CHlsSegmnter *)pCtx;
	if(pHttpLiveSgmnt) {
		res =  pHttpLiveSgmnt->WriteFrameData(pData, nLen,  fVideo, ulFlags,  ulPtsMs);
	} else {
		JDBG_LOG(CJdDbg::LVL_ERR,("httpliveWriteData: Invalid Segmenter"));
		res = -1;
	}
...
void CHlsPublishS3::UpLoadBroadcastPlayList()
{
	...
	int nSize = m_SegmentList.size();
	if(nSize > m_nBcastSegmentCount) {
		nSegmentCount = m_nBcastSegmentCount;
	} else {
		nSegmentCount = nSize;
	}
	...
	nLen = GetM3u8PlayListFromSegmentList(&m_SegmentList, buffer, this->m_pszM3u8BaseName, PLAYLIST_BUFFER_SIZE, nBcastFrontCache, nSegmentCount, 1, 0);
...
void CHlsPublishS3::UpdateSlidingWindow()
{
	int nDeleteIdx = 0;
	CSegmentInf *pSegmentInf = new CSegmentInf(m_nSegmentIndex, m_nSegmentDurationMs, m_fDiscont);
	m_SegmentList.push_back(pSegmentInf);

	if(m_SegmentList.size() > m_nBcastSegmentCount + m_nBcastFrontCache + m_nBcastBackCache) {
		CSegmentInf *pSegmentInf = m_SegmentList.front();
		m_SegmentList.pop_front();
...
DWORD CHlsPublishS3::Process()
{
	...
	m_pszM3u8BaseName = CM3u8Helper::GetPlayListBaseName(m_pszM3u8File);

	if(0) {
		char szHtmlFile[MAX_FILE_NAME];
		buffer = (char *)malloc(PLAYLIST_BUFFER_SIZE);
		if(buffer) {
			sprintf(szHtmlFile,"%s.html", m_pszM3u8BaseName);
			int nLen = GetHtmlCamHttpLive(buffer, m_pszM3u8BaseName, PLAYLIST_BUFFER_SIZE, 640, 480);
	...
	while(!fDone){
		...
		/* Segment Available */
		GetSegmentNameFromIndex(szTsFileName, m_pszM3u8BaseName, m_nSegmentIndex);
	...
	if(SegmentCount() > 0 && !m_fLiveOnly)
		UpLoadFullPlayList(m_pszM3u8BaseName, m_nSegmentStartIndex, SegmentCount());
Exit:
	m_fRunState = 0;
			...
			)
{
	...
	if(nDestType == UPLOADER_TYPE_S3 || nDestType == UPLOADER_TYPE_DISC) {
		...
		pHlsUpload = new CHlsPublishMemFile(pSegmenter, pszSrcM3u8File, pszBucket/*ServerRoot*/, pszParentFolder, fLiveOnly, nSegmentDurationMs);
	}
