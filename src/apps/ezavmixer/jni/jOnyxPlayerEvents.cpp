...
COnyxPlayerEvents::COnyxPlayerEvents(JNIEnv* env,jobject javaReceiver)
{
	...
	g_deviceClass = env->FindClass("com/mcntech/ezavmixer/OnyxPlayerApi");
...
{
	__android_log_vprint(ANDROID_LOG_INFO  , "[ezavmixer]", msg, args);
	return 0;
}
...
bool COnyxPlayerEvents::onRtspServerStatus(const char *szUrl, int nState, int nStrmInTime, int nStrmOutTime, int nLostBufferTime)
{
	...
	if(onyxApi != NULL) {
		...
		if(callback != NULL) {
			...
			JDBG_LOG(CJdDbg::LVL_ERR, ("Failed to find onMpdPublishStatus on com/mcntech/ezavmixer/OnyxPlayerApi"));
		}

	} else {
		JDBG_LOG(CJdDbg::LVL_ERR, ("Failed to find com/mcntech/ezavmixer/OnyxPlayerApi"));
	}
	safeDetach();
	return true;
}
...
bool COnyxPlayerEvents::onDiscoverRemoteNode(char *url)
{
	...
	if(onyxApi != NULL) {
		...
		if(callback != NULL) {
			...
			JDBG_LOG(CJdDbg::LVL_ERR, ("Failed to find onDiscoverRemoteNode on com/mcntech/ezavmixer/OnyxPlayerApi"));
		}
	} else {
		JDBG_LOG(CJdDbg::LVL_ERR, ("Failed to find com/mcntech/ezavmixer/OnyxPlayerApi"));
	}
