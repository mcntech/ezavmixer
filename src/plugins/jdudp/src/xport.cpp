...
{
public:
    CAC3ParseCtx():CParseCtx(8*1024) {}

    int		first_header = FALSE;
    int		second_header = FALSE;
    int		audio_synced = FALSE;
    int		first_synced = FALSE;
    unsigned int	parse = 0;
    unsigned int	header_parse = 0;
    unsigned int	frame_size = 0;
    unsigned int	frame_size_check = 0;
    unsigned char	frame_buffer[128][3840 + 8];
    unsigned char	frame_buffer_start = 0x0b;
    unsigned int	frame_buffer_index = 0;
    unsigned int	frame_buffer_count = 0;
    unsigned int	frame_buffer_length[128];
    unsigned long long	frame_buffer_pts[128];
    unsigned long long	current_pts;
    unsigned long long	current_pts_saved;
    unsigned int	current_pts_valid = FALSE;
    unsigned int	audio_sampling_rate;
    unsigned int	audio_bitrate;
    unsigned int	audio_bsid;
    unsigned int	audio_bsmod;
    unsigned int	audio_acmod;
...
class CAACParseCtx : public CParseCtx {
public:
	CAACParseCtx():CParseCtx(8*1024) {}
    int		audio_synced = FALSE;
    unsigned int	parse = 0;
...
public:
	...
	unsigned int	frame_size_check = 0;
	unsigned char	frame_buffer[128][3840 + 8];
	unsigned char	frame_buffer_start = 0xff;
	unsigned int	frame_buffer_index = 0;
	unsigned int	frame_buffer_count = 0;
	unsigned int	frame_buffer_length[128];
	unsigned long long	frame_buffer_pts[128];
	...
	unsigned int	audio_mode_ext;
	unsigned int	audio_copyright;
...
public:
	...
	unsigned int	sample;
	unsigned int	first_header_dump = FALSE;
	unsigned int	extra_bytes = 0;
	unsigned char	extra_bytes_buffer[4];
	unsigned char	temp_buffer[188];
	unsigned char	null_bytes[4] = {0, 0, 0, 0};

};

class CMPEG2ParseCtx : public CParseCtx
{
public:
	CMPEG2ParseCtx():CParseCtx(256*1024) {}
	int		first = TRUE;
	int		first_sequence = FALSE;
	int		first_sequence_dump = FALSE;
	int		look_for_gop = FALSE;
	int		gop_found = FALSE;
	unsigned int	parse = 0;
	unsigned int	picture_parse = 0;
	unsigned int	extension_parse = 0;
	unsigned int	picture_coding_parse = 0;
	unsigned int	sequence_header_parse = 0;
	unsigned int	sequence_extension_parse = 0;
	unsigned int	picture_size = 0;
	unsigned int	picture_count = 0;
	unsigned int	time_code_field = 0;
	unsigned int	time_code_rate = 1;
	long double	frame_rate = 1.0;
	unsigned char	header[3] = {0x0, 0x0, 0x1};
	unsigned char	gop_header[9] = {0xb8, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00};
	unsigned int	progressive_sequence;
	unsigned long long	first_pts;
	unsigned int	first_pts_count = 0;
	unsigned int	extra_byte = FALSE;
	unsigned int	last_temporal_reference = 0;
	unsigned int	last_gop_temporal_reference = 0;
	unsigned int	running_average_start = 0;
	unsigned int	running_average_count = 0;
	unsigned int	running_average_frames = 0;
	unsigned int	running_average_samples[1024];
	unsigned int	running_average_fields[1024];
	void parse_video(MpegTsDemuxCtx *pCtx, unsigned char *es_ptr, unsigned int length,
					 unsigned long long pts, unsigned int dts);
};

class CH264ParseCtx : public CParseCtx
{
public:
	CH264ParseCtx():CParseCtx(4*1024*1024){
	}
	virtual ~CH264ParseCtx()
	{
	}
...
class CH265ParseCtx : public CH264ParseCtx  {
public:
	CH265ParseCtx() : CH264ParseCtx() {
	}
    void parse_video(MpegTsDemuxCtx *pCtx, unsigned char *es_ptr, unsigned int length,
                     unsigned long long pts, unsigned long long dts);
};
...
public:
    ...
    CPsiSectionBuff(const CPsiSectionBuff &p)
    {
        memcpy(mBuff,p.mBuff, p.mWr);
        mWr = p.mWr;
    }
...
public:
    ...
    unsigned long long	clkPcr;
    unsigned long long	clkPrevDiscont;

	unsigned long long	video_pts;
	unsigned long long	video_dts;
    ...
    unsigned long long  stream_rate;
...
{
    if(pPidCtx) {
        unsigned long long stream_rate = 0;
        unsigned long long saveClkPcr = pPidCtx->clkPcr;
...
void savePmtData(MpegTsDemuxCtx *pCtx, CPsiSectionBuff *pPmt)
{

/*
    CPsiSectionBuff *pPmtTmp = getPmtBuffer(pCtx,pCtx->pid);
    *pPmtTmp = *pPmt;
*/
}
...
int isVideoPid(MpegTsDemuxCtx *pCtx, int nPid) {
    ...
    if (streamType == 0x1 || streamType == 0x2 || streamType == 0x80 || streamType == 0x1b || streamType == 0x24) {
       return 1;
    }
    return 0;
}
...
int demuxSetOption(StrmCompIf *pComp, int nCmd, char *pOptionData)
{
	...
	switch(nCmd)
	{
        ...
        break;
        case DEMUX_CMD_GET_PID_STAT:
        {
            PidStatT *pStat = (PidStatT *)pOptionData;
            CPidCtx *pPidCtx = pCtx->mPidCtxs[pStat->nPid];
            // TODO
            calcStreamRate(pCtx, pPidCtx);
            pStat->bitrate = pPidCtx->stream_rate;
        }
        break;
...
int demuxSetOutputConn(StrmCompIf *pComp, int nPid, ConnCtxT *pConn)
{
    ...
    switch (nStrmType) {
        ...
        case 0x02:
        case 0x80:
            pX = new CMPEG2ParseCtx();
            sprintf(outputName, "stream_pid_%d.mp2v", nPid);
            break;
        case 0x1b:
            pX = new CH264ParseCtx();
            sprintf(outputName, "stream_pid_%d.h264", nPid);
            break;
		case 0x24:
			pX = new CH265ParseCtx();
			sprintf(outputName, "stream_pid_%d.h265", nPid);
			break;
        case 0x81:
            pX = new CAC3ParseCtx();
        ...
        case 0x03:
        case 0x04:
            pX= new CMP2AudParseCtx();
            sprintf(outputName, "stream_pid_%d.mp2a", nPid);
            break;
        case 0x0F:
            pX= new CAACParseCtx();
            sprintf(outputName, "stream_pid_%d.aac", nPid);
            break;
    }
    if (pX != NULL) {
        pX->pConn = pConn;
...
int demuxStart(StrmCompIf *pComp)
{
	...
	if (pthread_create (&pCtx->threadidDemux, NULL, (thrdStartFcnPtr)threadDemux, pCtx) != 0)	{
		...
		pthread_setname_np(pCtx->threadidDemux, "demux");
	}
#endif
	JdDbg(CJdDbg::DBGLVL_TRACE, ("Leave"));
    return 0;
...
void CAC3ParseCtx::parse_ac3_audio(MpegTsDemuxCtx *pCtx, unsigned char *es_ptr, unsigned int length, unsigned long long pts, unsigned int first_access_unit, int end_of_frame)
{
#if 1
	unsigned int	i, j;
    int numChannels = 2;
    ...
    if(pCtx->parse_only == FALSE)  {
        if(audio_synced == TRUE)  {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if ((parse & 0xfff0) == 0x0000fff0) {
                    unsigned char *buffer = es_ptr;
                    int sampleIndex = (buffer[0] & 0x3C) >> 2;
                    int channelCfg = (buffer[0] & 0x01) << 2 | (buffer[1] & 0xC0) >> 6;
                    if(sampleIndex < 12 && channelCfg > 0 && channelCfg < 8) {
                        audio_synced = TRUE;
                        AudParam audParam = {0};
                        audParam.nNumChannels = channelCfg == 7 ? 8 : channelCfg;
                        audParam.nSampleRate = SAMPLE_RATES[sampleIndex];
                        NotifyFrmatChange(pCtx, pCtx->pid, 0x0F, (unsigned char *) &audParam,
                                          sizeof(audParam));
                    }
                }
            }
...
void CMPEG2ParseCtx::parse_video(MpegTsDemuxCtx *pCtx, unsigned char *es_ptr, unsigned int length,
								 unsigned long long pts, unsigned int dts)
{
	unsigned int	i, j;
...
void CH264ParseCtx::parse_video(MpegTsDemuxCtx *pCtx, unsigned char *es_ptr, unsigned int length,
								unsigned long long pts, unsigned long long dts)
{
	unsigned int	i;
...
void CH265ParseCtx::parse_video(MpegTsDemuxCtx *pCtx, unsigned char *es_ptr, unsigned int length,
                                unsigned long long pts, unsigned long long dts)
{
    unsigned int	i;
    ...
    for(i = 0; i < length; i++) {
        parse = (parse << 8) + *es_ptr++;
        if ((parse & 0xffffff00) == 0x00000100) {
            int startCode = (parse >> 1) & 0x3F;
            switch (startCode) {
                case HEVC_NAL_SPS:
                    NotifyFrmatChange(pCtx, pCtx->pid, 0x24, es_ptr - 1,
                                      length - (es_ptr - start_es_ptr));
                    first_sequence = TRUE;
                    break;
				...
				if (pCtx->xport_header_parse != 0)  {
					...
					switch (pCtx->xport_header_parse)  {
						...
						case 1:
                            ...
                            pPidCtx = getPidCtx(pCtx, pCtx->pid);
                            pPidCtx->pcr_bytes += 188; // TODO
							break;
						case 0:
							temp = buffer[i];
					...
					if ((pCtx->adaptation_field_length - pCtx->adaptation_field_parse) == 1)  {
						...
						if (pCtx->pcr_parse == 0 /*&& pCtx->pid == pCtx->pcr_pid*/)  {
                            //CPidCtx *pPidCtx = getPidCtx(pCtx, pCtx->pid);
...
*/
						...
						if (pCtx->pat_section_length == 0)  {
							...
							if (pCtx->pat_section_number == pCtx->pat_last_section_number)  {
								for (k = 0; k < (pCtx->pat_offset - 4); k+=4)  {
									...
									pgm_pid = pCtx->program_association_table[k+2] << 8;
						...
						if (pCtx->pmt_section_length == 0)  {
							...
							if (pCtx->pmt_section_number == pCtx->pmt_last_section_number)  {
								for (k = 0; k < (pCtx->pmt_offset - 4); k+=5)  {
									...
									if (pCtx->pmt_stream_type == 0x1 || pCtx->pmt_stream_type == 0x2 || (pCtx->pmt_stream_type == 0x80 && pCtx->hdmv_mode == FALSE) || pCtx->pmt_stream_type == 0x1b || pCtx->pmt_stream_type == 0x24 || pCtx->pmt_stream_type == 0xea)  {
					...
					if (pCtx->video_xfer_state == TRUE)  {
						...
						if(streamType == 0x1 || streamType == 0x2 || streamType == 0x80)  {
							CMPEG2ParseCtx *pParser = (CMPEG2ParseCtx *)getParseCtx(pCtx, pCtx->pid);
							if(pParser != NULL)
								pParser->parse_video(pCtx, &buffer[i], xfer_length, pPidCtx->video_pts, pPidCtx->video_dts);
						}
						else if(streamType == 0x1b)  {
							CH264ParseCtx *pParser = (CH264ParseCtx *)getParseCtx(pCtx, pCtx->pid);
							if(pParser != NULL)
								pParser->parse_video(pCtx, &buffer[i], xfer_length, pPidCtx->video_pts, pPidCtx->video_dts);
						}
						else if(streamType == 0x24)  {
							CH265ParseCtx *pParser = (CH265ParseCtx *)getParseCtx(pCtx, pCtx->pid);
							if(pParser != NULL)
								pParser->parse_video(pCtx, &buffer[i], xfer_length, pPidCtx->video_pts, pPidCtx->video_dts);
						}
						else  {
						...
						if ((pCtx->video_parse >= 0x000001e0 && pCtx->video_parse <= 0x000001ef) || pCtx->video_parse == 0x000001fd)  {
							...
							switch (pCtx->video_pts_parse)  {
								...
								case 0:
									...
									pCtx->video_pes_header[pCtx->video_pes_header_index++] = pCtx->video_parse & 0xff;
									pPidCtx->video_pts = pCtx->video_temp_pts;
									if (pPidCtx->video_pts > pPidCtx->last_video_pts)  {
										pPidCtx->last_video_pts = pPidCtx->video_pts;
									}
									pPidCtx->video_dts = 0;
									...
									pPidCtx->last_video_pts_diff = pPidCtx->video_pts - pCtx->prev_video_dts;
									pCtx->prev_video_dts = pPidCtx->video_pts;
							...
							switch (pCtx->video_pts_dts_parse)  {
								...
								case 0:
									...
									pPidCtx->last_video_pts_diff = pCtx->video_temp_dts - pCtx->prev_video_dts;
					...
					if (pX->audio_xfer_state == TRUE)  {
						...
						if (pCtx->demux_audio == TRUE)  {
                            ...
                            int streamType = getStreamType(pCtx, pCtx->pid);
							if(streamType == 0x81) {
                                CAC3ParseCtx *pAudParse = (CAC3ParseCtx *) getParseCtx(pCtx,
                                                                                       pCtx->pid);
                                if (pAudParse != NULL) {
                                    //pAudParse->parse_ac3_audio(pCtx, &buffer[i], xfer_length, pPidCtx->audio_pts, pCtx->first_audio_access_unit, end_of_frame);
                                }
							}
                            else if( streamType == 0x6)  {
                                CDVBParseCtx *pAudParse = (CDVBParseCtx *)getParseCtx(pCtx, pCtx->pid);
                                if(pAudParse!=NULL)
                                    pAudParse->parse_dvb_subtitle(pCtx, &buffer[i], xfer_length, pPidCtx->audio_pts, pCtx->first_audio_access_unit, end_of_frame);
                            }
							else if(streamType == 0x3 || streamType == 0x4)  {
								CMP2AudParseCtx *pAudParse = (CMP2AudParseCtx *)getParseCtx(pCtx, pCtx->pid);
								if(pAudParse!=NULL)
									pAudParse->parse_mp2_audio(pCtx, &buffer[i], xfer_length, pPidCtx->audio_pts, pCtx->first_audio_access_unit, end_of_frame);
							}
							else if(streamType == 0x80)  {
								CPCMParseCtx *pAudParse = (CPCMParseCtx *)getParseCtx(pCtx, pCtx->pid);
								if(pAudParse!=NULL)
									pAudParse->parse_lpcm_audio(pCtx, &buffer[i], xfer_length, pPidCtx->audio_pts, pCtx->first_audio_access_unit, pCtx->audio_lpcm_header_flags);
							}
							else if(streamType == 0x0f)  {
								CAACParseCtx *pAudParse = (CAACParseCtx *)getParseCtx(pCtx, pCtx->pid);
								if(pAudParse!=NULL)
									pAudParse->parse_aac_audio(pCtx, &buffer[i], xfer_length, pPidCtx->audio_pts, pCtx->first_audio_access_unit, end_of_frame);
							}
							else  {
								if(pCtx->parse_only == FALSE)  {
									WriteData(pCtx, &buffer[i], 1, xfer_length, pCtx->pid);
						...
						if (((pCtx->audio_parse >= 0x000001c0 && pCtx->audio_parse <= 0x000001df) /*&& (pCtx->audio_stream_type == 0x3 || pCtx->audio_stream_type == 0x4 || pCtx->audio_stream_type == 0x6 || pCtx->audio_stream_type == 0x0f)*/) || pCtx->audio_parse == 0x000001bd || pCtx->audio_parse == 0x000001fd || pCtx->audio_parse == 0x000001fa)  {
							...
							switch (pCtx->audio_pts_parse)  {
								...
								case 0:
									...
									pCtx->audio_pes_header[pCtx->audio_pes_header_index++] = pCtx->audio_parse & 0xff;
									pPidCtx->audio_pts = pCtx->audio_temp_pts;
									if (pPidCtx->audio_pts > pPidCtx->last_audio_pts)  {
                                        pPidCtx->last_audio_pts = pPidCtx->audio_pts;
									}
									pCtx->first_audio_access_unit = TRUE;
									if(pCtx->dump_audio_pts == TRUE)  {
										DBG_MSG("Audio PTS = %d, %d\r\n", (unsigned int)pPidCtx->audio_pts, (unsigned int)(pPidCtx->audio_pts - pCtx->prev_audio_pts));
									}
									pCtx->last_audio_pts_diff = pPidCtx->audio_pts - pCtx->prev_audio_pts;
									pCtx->prev_audio_pts = pPidCtx->audio_pts;
									if (pCtx->audio_pes_header_length == 0)  {
